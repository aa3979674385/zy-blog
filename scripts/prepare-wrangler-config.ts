...
export function prepareWranglerConfig(env: EnvMap): "custom_domain" | "routes" {
  ...
  let content = prepareWranglerConfigContent({
  ...
  // 公开给客户端的 CAPTCHA 变量（vars，非 secrets）：在 CI 中从 GITHUB_ENV 注入后，
  // 替换掉 wrangler.example.jsonc 里的占位符，否则 Worker 运行时拿到的会是字面量占位符。
  if (env.VITE_TURNSTILE_SITE_KEY?.trim()) {
    content = content.replaceAll("YOUR_TURNSTILE_SITE_KEY", env.VITE_TURNSTILE_SITE_KEY.trim());
  }
