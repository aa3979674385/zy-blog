...
export const Route = createFileRoute("/admin/")({
  ...
  loader: async ({ context }) => {
    // 仅当拥有 dashboard.view 时才预取仪表盘数据，避免无权限管理员加载时服务端抛错
    const session = (context as unknown as { session?: { user?: PermissionSubject } })
      .session;
    if (session?.user && hasPermission(session.user, "dashboard.view")) {
...
function DashboardOverview() {
  ...
  const canViewDashboard = hasPermission(mySubject, "dashboard.view");
  const canRefreshCache = hasPermission(mySubject, "cache.manage");
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-300 mx-auto">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    disabled={refreshDashboardCacheMutation.isPending}
                  ...
                  >
                    <RefreshCw
                      size={14}
                      className={
                        refreshDashboardCacheMutation.isPending
                          ? "animate-spin"
                          : ""
                      }
                    />
      ...
      {canViewDashboard ? (
        ...
        <div className="border border-border/30 bg-background p-12 text-center">
...
function DashboardContent({ range }: { range: DashboardRange }) {
