...
type ImageTab = "upload" | "library" | "link";
...
const InsertModalInternal: React.FC<InsertModalProps> = ({
  ...
  // 图片模式：分栏状态
  const [imgTab, setImgTab] = useState<ImageTab>("upload");
  ...
  return createPortal(
    ...
    >
      ...
      >
              ...
              <div className="flex border-b border-border/50 shrink-0">
                {(
                  [
                    ...
                    { key: "link", label: "外部链接" },
                  ] as Array<{ key: ImageTab; label: string }>
                ).map((t) => (
                  <button
                    key={t.key}
                    type="button"
                    onClick={() => setImgTab(t.key)}
                    className={cn(
              ...
              {imgTab === "upload" && (
                <div className="flex-1 overflow-y-auto p-6 bg-muted/5">
                  ...
                  <button
                    ...
                    className={cn(
                      "flex w-full flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed py-14 text-muted-foreground transition-colors hover:border-primary hover:text-foreground disabled:opacity-60",
                  ...
                  >
                    {uploading ? (
                      ...
                      <>
                        ...
                        <span className="text-sm">点击选择图片，或拖拽到此处</span>
        ...
        {/* Actions Footer（上传 Tab 上传成功即插入，无需确认按钮） */}
        <div className="flex items-center justify-end gap-0 border-t border-border/50">
          ...
          {(activeType === "LINK" ||
            (activeType === "IMAGE" && imgTab !== "upload")) && (
            <button
              type="button"
