...
export function usePostActions({
  ...
  // ⚠️ 约定：PostEditorData 新增可编辑字段时，必须同步加到下面的比较列表，
  //    否则该字段的改动不会让发布按钮激活（isPostDirty 始终为 false）。
  const isDirty = useMemo(() => {
    ...
    return (
      ...
      // 封面图：留空与 null 视为等价，统一用 ?? null 归一化比较
      (post.coverImage ?? null) !== (kvSnapshot.coverImage ?? null) ||
    ...
    !compareTags(post.categoryIds, kvSnapshot.categoryIds)
  );
