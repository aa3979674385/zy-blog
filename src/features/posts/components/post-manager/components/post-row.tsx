        ...
        {post.categories && post.categories.length > 0 ? (
          <div className="flex flex-wrap gap-1">
            {post.categories.map((cat) => (
              <span
                key={cat}
                className="inline-flex items-center rounded-sm bg-foreground/5 border border-foreground/15 px-1.5 py-px text-[10px] font-mono font-medium text-foreground/80"
              >
                {cat}
              </span>
            ))}
