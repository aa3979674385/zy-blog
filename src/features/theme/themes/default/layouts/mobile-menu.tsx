        ...
        <nav className="flex-1 flex flex-col justify-center space-y-6 md:space-y-8 font-mono">
          {(() => {
            ...
            navOptions.forEach((item) => {
              entries.push({ item, depth: 0, animIdx: ai++ });
              item.children?.forEach((child) =>
                entries.push({ item: child, depth: 1, animIdx: ai++ }),
              );
            });
            return entries.map(({ item, depth, animIdx }) => {
              const pad = depth === 0 ? "" : "pl-10";
              const size = depth === 0 ? "text-3xl md:text-5xl" : "text-2xl md:text-3xl";
              const delay = isOpen ? `${50 + animIdx * 50}ms` : "0ms";
              return item.external ? (
                <a
                  ...
                  rel="noreferrer"
                  onClick={onClose}
                  className={`group flex items-center gap-4 transition-all duration-500 ${pad} ${size} ${
                    isOpen
                ...
                >
                  ...
                  <span className="font-bold tracking-tight text-muted-foreground transition-colors group-hover:text-foreground">
                    {item.label}
                  </span>
                </a>
              ) : (
