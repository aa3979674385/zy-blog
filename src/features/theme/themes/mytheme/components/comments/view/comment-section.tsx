...
import { AsyncCommentEditor } from "../editor/async-comment-editor";
...
export function FuwariCommentSection({ postId }: FuwariCommentSectionProps) {
  ...
  const {
    ...
    ensureVerified,
  } = useTurnstile("comment", { lazy: true });

  const [isVerifying, setIsVerifying] = useState(false);

  // 懒加载：聚焦到评论输入框（contenteditable）才激活验证框
  const handleEditorFocus = (e: FocusEvent<HTMLElement>) => {
  ...
  return (
    <div className="space-y-6" onFocusCapture={handleEditorFocus}>
      ...
      {session ? (
        <AsyncCommentEditor
          ...
          isSubmitting={isVerifying || (isCreating && !replyTarget)}
        />
      ) : (
      ...
      <FuwariCommentList
        ...
        isSubmittingReply={isVerifying || isCreating}
